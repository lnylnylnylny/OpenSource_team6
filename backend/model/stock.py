...
class Stock(Base):
    ...
    symbol = Column(String, unique=True, nullable=False)  # 005930
    name = Column(String, nullable=False)
    market = Column(String)  # KOSPI, KOSDAQ
    asset_type = Column(String)
    last_price = Column(Numeric(precision=15, scale=2))
    prev_close = Column(Numeric(precision=15, scale=2))
    change_rate = Column(Numeric(precision=8, scale=2))
...
class Quote(Base):
    ...
    __table_args__ = (
        ...
        Index(
            "idx_stock_time_desc", "stock_id", "quote_time", mysql_length=255
        ),  # DESC 인덱스
    )
...
class Order(Base):
    ...
    __table_args__ = (Index("idx_user_stock_status", "user_id", "stock_id", "status"),)
...
class Trade(Base):
    ...
    trade_time = Column(DateTime(timezone=True), default=func.now())
